...
function create_cosine_lobe(center_position, end_position, exponent, segments, stroke_width) {
    let [cx, cy] = center_position;
    let [ex, ey] = end_position;
    let dx = ex - cx;
    let dy = ey - cy;
    ...
    for (let i = 1; i < segments; i++) {
        let t = i / segments;
        let theta = -Math.PI/2 + t * Math.PI;
        let r = radius * Math.pow(Math.max(Math.cos(theta), 0), exponent);
        let x = cx + dx * r * Math.cos(theta) + rx * r * Math.sin(theta);
        let y = cy + dy * r * Math.cos(theta) + ry * r * Math.sin(theta);
        points.push([x, y]);
    }
    points.push([cx, cy]);
    return [points, create_line(points, stroke_width)];
}
...
void main(){
    vec3 view_vector = normalize(camera_pos - world_position);
    vec3 normal = normalize(world_normal);
    float normal_dot_view = max(dot(normal, view_vector), 0.0);
    vec3 fresnel = fresnel_schlick(normal_dot_view, base_reflectance);
    vec3 display = pow(clamp(fresnel, 0.0, 1.0), vec3(0.5));
    frag_color = vec4(display, 1);
}`);


ctx.shaders["shader_diffuse"] = ctx.create_shader(`#version 300 es
layout(location = 0) in vec3 position_attrib;
layout(location = 1) in vec3 normal_attrib;
layout(location = 2) in vec2 texcoord_attrib;
layout(location = 3) in vec3 tangent_attrib;
...
void main(){
    vec3 light_dir = normalize(light_direction);
    vec3 origin_relative_position = world_position - offset;
    vec3 view_vector = normalize(camera_pos - origin_relative_position);
    vec3 normal = normalize(world_normal);
    vec3 halfway_vector = normalize(light_dir + view_vector);

    float n_dot_l = max(dot(normal, light_dir), 0.0);
    float n_dot_v = max(dot(normal, view_vector), 0.0);
    float l_dot_v = dot(light_dir, view_vector);
    ...
    if(diffuse_type == 0.0){
        diffuse = ONE_OVER_PI * n_dot_l;
    }
    else if(diffuse_type == 1.0){
        ...
        float t = (s > 0.0) ? max(n_dot_l, n_dot_v) : 1.0;

        diffuse = (a + b * max(0.0, s / t)) * ONE_OVER_PI * n_dot_l;
    }
...
ctx.scenes = {
    ...
    "scene_specular": {id: "scene_specular", el: null, ratio: 2.6, camera: null, dragging_rect: null, draggable_rects: {},
        camera: {
            fov: 70, z_near: 0.1, z_far: 1000,
            position: [0, 0, 0], rotation: [0, 0, 0],
            up_vector: [0, 1, 0],
    ...
    "scene_diffuse_lobe": {id: "scene_diffuse_lobe", el: null, ratio: 2.6, camera: null, dragging_rect: null, draggable_rects: {},
        camera: {
            fov: 70, z_near: 0.1, z_far: 1000,
            position: [0, 0, 0], rotation: [0, 0, 0],
            up_vector: [0, 1, 0],
    ...
    "scene_normal_distribution_function": {id: "scene_normal_distribution_function", el: null, ratio: 2.2, camera: null, dragging_rect: null, directional_light: [0.5, 0.5], draggable_rects: {"light": []},
        camera: {
            fov: 70, z_near: 0.1, z_far: 1000,
            position: [0, 0, 0], rotation: [0, 0, 0],
            up_vector: [0, 1, 0],
    ...
    "scene_diffuse": {id: "scene_diffuse", el: null, ratio: 3, camera: null, dragging_rect: null, directional_light: [0.5, 0.5], draggable_rects: {"light": []},
        camera: {
            fov: 70, z_near: 0.1, z_far: 1000,
            position: [0, 0, 0], rotation: [0, 0, 0],
            up_vector: [0, 1, 0],
    ...
    "scene_geometric_function": {id: "scene_geometric_function", el: null, ratio: 2.2, camera: null, dragging_rect: null, directional_light: [0.5, 0.5], draggable_rects: {"light": []},
        camera: {
            fov: 70, z_near: 0.1, z_far: 1000,
            position: [0, 0, 0], rotation: [0, 0, 0],
            up_vector: [0, 1, 0],
    ...
    "scene_fresnel_equation": {id: "scene_fresnel_equation", el: null, ratio: 2.2, camera: null, dragging_rect: null, directional_light: [0.5, 0.5], draggable_rects: {"light": []},
        camera: {
            fov: 70, z_near: 0.1, z_far: 1000,
            position: [0, 0, 0], rotation: [0, 0, 0],
            up_vector: [0, 1, 0],
    ...
    "scene_pbr_demo": {id: "scene_pbr_demo", el: null, ratio: 2.2, camera: null, dragging_rect: null, directional_light: [0.5, 0.5], draggable_rects: {"light": []},
        camera: {
            fov: 70, z_near: 0.1, z_far: 1000,
            position: [0, 0, 0], rotation: [0, 0, 0],
            up_vector: [0, 1, 0],
...
function handle_interaction_end(e) {
    for (let scene_id in ctx.scenes) {
        ...
        if(scene.is_dragging){
            ...
            scene.el.style.cursor = scene_uses_orbit_drag(scene) ? "grab" : "default";
        }
    }
}

function scene_uses_right_drag_light(scene) {
    return scene.hasOwnProperty("directional_light");
}

function scene_uses_orbit_drag(scene) {
    return scene_uses_right_drag_light(scene) || scene.draggable_rects.hasOwnProperty("scene");
}

function get_scene_cursor(scene, hovered_rect_id = null) {
    if(scene.is_dragging){
        if(scene.dragging_rect == "scene" || scene.dragging_rect == "light"){
            return "grabbing";
        }
        return "move";
    }

    if(hovered_rect_id == "scene" || hovered_rect_id == "light"){
        return "grab";
    }

    if(hovered_rect_id != null){
        return "move";
    }

    if(scene_uses_orbit_drag(scene)){
        return "grab";
    }
    ...
    for (let scene_id in ctx.scenes) {
        ...
        (function(scene_id, scene) {
            function handle_move(e) {
                ...
                let hovered_rect_id = null;

                for (let rect_id in scene.draggable_rects) {
                    if(rect_id != "scene" && rect_id != "light" && scene.draggable_rects[rect_id].object.state != "dragged"){
                ...
                for (let rect_id in scene.draggable_rects) {
                    ...
                    if (rect_id == "scene" || rect_id == "light" || coords.x >= rect[0] && coords.x <= rect[2] &&
                        coords.y >= rect[1] && coords.y <= rect[3]) {
                        hovered_rect_id = rect_id;
                        if(rect_id != "scene" && rect_id != "light" && scene.draggable_rects[rect_id].object.state != "dragged"){
                ...
                scene.el.style.cursor = get_scene_cursor(scene, hovered_rect_id);
            }

            function handle_start(e) {
                e.preventDefault();
                const is_touch = !!e.touches;
                const is_primary_button = is_touch || e.button === 0 || e.which === 1;
                const is_secondary_button = !is_touch && (e.button === 2 || e.which === 3);
                if (!is_primary_button && !is_secondary_button) return;

                const coords = get_event_coordinates(e, scene.el);

                if(scene_uses_right_drag_light(scene)){
                    scene.dragging_rect = is_secondary_button ? "light" : "scene";
                    scene.is_dragging = true;
                    scene.last_pos = [coords.x, coords.y];
                    scene.el.style.cursor = get_scene_cursor(scene, scene.dragging_rect);
                    return;
                }
                ...
                for (let rect_id in scene.draggable_rects) {
                    ...
                    if (rect_id == "scene" || rect_id == "light" || coords.x >= rect[0] && coords.x <= rect[2] &&
                        ...
                        scene.is_dragging = true;
                        scene.last_pos = [coords.x, coords.y];
                        if(rect_id != "scene" && rect_id != "light"){
                            scene.draggable_rects[rect_id].object.state = "dragged";
                        }
                        scene.el.style.cursor = get_scene_cursor(scene, rect_id);
                        break;
                    }
                }
            }
            ...
            function handle_end(e) {
                ...
                scene.el.style.cursor = get_scene_cursor(scene);
            }
            ...
            scene.el.removeEventListener("contextmenu", scene.event_listener_contextmenu);
            ...
            scene.event_listener_contextmenu = scene.el.addEventListener("contextmenu", handle_contextmenu);
        })(scene_id, scene);
    }
}
...
function handle_global_move(e) {
    ...
    for(let scene_id in ctx.scenes) {
        ...
        if(scene.dragging_rect == "light") {
            scene.directional_light = vec2_add(
                scene.directional_light,
                [-pos_delta[1] * delta_angle[1], pos_delta[0] * delta_angle[0]]
            );
        }
...
ctx.draw = function(drawable, custom_uniforms, custom_camera, custom_shader){
    ...
    gl.drawElements(gl.TRIANGLES, drawable.vertex_buffer.draw_count, gl.UNSIGNED_INT, 0);
}
...
ctx.reset_drawable_mesh = function(drawable, mesh){
    const gl = this.gl;
    if(drawable.vertex_buffer != null){
        gl.deleteVertexArray(drawable.vertex_buffer.vao);
        gl.deleteBuffer(drawable.vertex_buffer.vbo);
...
// scene_metals
// scene_non_metals
let plastic_sphere = ctx.create_drawable("shader_pbr", create_uv_sphere_tangent(1, 64, 64, true), [0, 0, 0], translate_3d([0, 0, 0]),
    [{ name: "position_attrib", size: 3 },
    { name: "normal_attrib", size: 3 },
    { name: "texcoord_attrib", size: 2 },
    { name: "tangent_attrib", size: 3 },]
...
let normal_distribution_bunny = ctx.create_drawable("shader_normal_distribution", null, [0, 0, 0], mat4_identity());
let normal_distribution_mesh = normal_distribution_sphere;
...
// scene_normal_distribution_function
// scene_diffuse
let roughness_oren_nayar = 0.5;
document.getElementById("roughness-input-oren-nayar").value = roughness_oren_nayar;
document.getElementById("roughness-input-oren-nayar").addEventListener("input", (e) => {
    roughness_oren_nayar = parseFloat(e.target.value);
});
document.getElementById("diffuse_roughness_zero").addEventListener("click", () => {
    roughness_oren_nayar = 0;
    document.getElementById("roughness-input-oren-nayar").value = roughness_oren_nayar;
    update_slider_background(document.getElementById("roughness-input-oren-nayar"));
});
let diffuse_sphere_gap = 0;
let diffuse_viewport_width_percent = 37.5;
const diffuse_labels_el = document.getElementById("scene-diffuse-labels");
let diffuse_spheres = [
    {name: "lambert", label: "Lambert", diffuse_type: 0},
    {name: "oren_nayar", label: "Oren-Nayar", diffuse_type: 1},
].map((diffuse_sphere) => ({
    ...diffuse_sphere,
    sphere_drawable: ctx.create_drawable("shader_diffuse", create_uv_sphere_tangent(1, 64, 64, true), [0, 0, 0], translate_3d([0, 0, 0]),
        [{ name: "position_attrib", size: 3 },
        { name: "normal_attrib", size: 3 },
        { name: "texcoord_attrib", size: 2 },
        { name: "tangent_attrib", size: 3 },]
    ),
    bunny_drawable: ctx.create_drawable("shader_diffuse", null, [0, 0, 0], translate_3d([0, 0, 0])),
    current_drawable: null
}));
for(let diffuse_sphere of diffuse_spheres){
    diffuse_sphere.current_drawable = diffuse_sphere.sphere_drawable;
}
diffuse_labels_el.innerHTML = diffuse_spheres.map((diffuse_sphere) =>
    `<div style="text-align: center;">${diffuse_sphere.label}</div>`
).join("");
// scene_diffuse
...
let geometric_function_bunny = ctx.create_drawable("shader_geometric_function", null, [0, 0, 0], mat4_identity());
let geometric_function_mesh = geometric_function_sphere;
...
let fresnel_equation_bunny = ctx.create_drawable("shader_fresnel", null, [0, 0, 0], mat4_identity());
let fresnel_equation_mesh = fresnel_equation_bunny;
let base_reflectance = [0.0, 0.0, 0.0];
...
let pbr_demo_bunny = ctx.create_drawable("shader_pbr_demo", null, [0, 0, 0], mat4_identity());
let pbr_demo_mesh = pbr_demo_sphere;
...
for(let i = 0; i < grid_size; i++){
    for(let j = 0; j < grid_size; j++){
        let metallic = remap_value(i, 0, grid_size-1, 0, 1);
        let roughness = remap_value(j, 0, grid_size-1, 0.1, 1);
        let transform = translate_3d([pbr_demo_grid_spacing*(i-1.5), pbr_demo_grid_spacing*(j-1.5), 0]);
        let sphere_drawable = ctx.create_drawable("shader_pbr_demo", create_uv_sphere_tangent(1, 64, 64, true), [0, 0, 0], transform,
                [{ name: "position_attrib", size: 3 },
                { name: "normal_attrib", size: 3 },
                { name: "texcoord_attrib", size: 2 },
                { name: "tangent_attrib", size: 3 },]
            );
        let bunny_drawable = ctx.create_drawable("shader_pbr_demo", null, [0, 0, 0], transform);
        pbr_demo_grid_spheres.push({
            sphere_drawable: sphere_drawable,
            bunny_drawable: bunny_drawable,
            current_drawable: sphere_drawable,
            metallic: metallic,
            roughness: roughness
        });
    }
...
const scene_mesh_toggle_states = {
    "scene_normal_distribution_function": {
        get_kind: () => normal_distribution_mesh == normal_distribution_bunny ? "bunny" : "sphere",
        set_kind: (kind) => {
            normal_distribution_mesh = kind == "bunny" ? normal_distribution_bunny : normal_distribution_sphere;
        }
    },
    "scene_diffuse": {
        get_kind: () => diffuse_spheres[0].current_drawable == diffuse_spheres[0].bunny_drawable ? "bunny" : "sphere",
        set_kind: (kind) => {
            for(let diffuse_sphere of diffuse_spheres){
                diffuse_sphere.current_drawable = kind == "bunny" ? diffuse_sphere.bunny_drawable : diffuse_sphere.sphere_drawable;
            }
        }
    },
    "scene_geometric_function": {
        get_kind: () => geometric_function_mesh == geometric_function_bunny ? "bunny" : "sphere",
        set_kind: (kind) => {
            geometric_function_mesh = kind == "bunny" ? geometric_function_bunny : geometric_function_sphere;
        }
    },
    "scene_fresnel_equation": {
        get_kind: () => fresnel_equation_mesh == fresnel_equation_bunny ? "bunny" : "sphere",
        set_kind: (kind) => {
            fresnel_equation_mesh = kind == "bunny" ? fresnel_equation_bunny : fresnel_equation_sphere;
        }
    },
    "scene_pbr_demo": {
        get_kind: () => pbr_demo_mesh == pbr_demo_bunny ? "bunny" : "sphere",
        set_kind: (kind) => {
            pbr_demo_mesh = kind == "bunny" ? pbr_demo_bunny : pbr_demo_sphere;
        }
    },
    "scene_pbr_demo_grid": {
        get_kind: () => pbr_demo_grid_spheres[0].current_drawable == pbr_demo_grid_spheres[0].bunny_drawable ? "bunny" : "sphere",
        set_kind: (kind) => {
            for(let sphere of pbr_demo_grid_spheres){
                sphere.current_drawable = kind == "bunny" ? sphere.bunny_drawable : sphere.sphere_drawable;
            }
        }
    }
};

function update_scene_mesh_toggle_buttons(scene_id) {
    let toggle = document.querySelector(`.scene-mesh-toggle[data-mesh-target="${scene_id}"]`);
    if(toggle == null) return;
    let current_kind = scene_mesh_toggle_states[scene_id].get_kind();
    for(let button of toggle.querySelectorAll("[data-mesh-kind]")){
        button.classList.toggle("active", button.dataset.meshKind == current_kind);
    }
}

function block_scene_mesh_toggle_drag(e) {
    e.preventDefault();
    e.stopPropagation();
}

function setup_scene_mesh_toggle(scene_id) {
    let toggle = document.querySelector(`.scene-mesh-toggle[data-mesh-target="${scene_id}"]`);
    if(toggle == null) return;
    for(let button of toggle.querySelectorAll("[data-mesh-kind]")){
        button.addEventListener("mousedown", block_scene_mesh_toggle_drag);
        button.addEventListener("touchstart", block_scene_mesh_toggle_drag);
        button.addEventListener("contextmenu", block_scene_mesh_toggle_drag);
        button.addEventListener("click", () => {
            scene_mesh_toggle_states[scene_id].set_kind(button.dataset.meshKind);
            update_scene_mesh_toggle_buttons(scene_id);
        });
    }
    update_scene_mesh_toggle_buttons(scene_id);
}

for(let scene_id in scene_mesh_toggle_states){
    setup_scene_mesh_toggle(scene_id);
}
// scene_pbr_demo_grid
...
// scene_roughness_macro
// scene_specular
let diffuse_specular_line_y = -0.4;
let diffuse_specular_line = ctx.create_drawable("shader_basic",
        create_line([[-2, diffuse_specular_line_y, 0], [2, diffuse_specular_line_y, 0]], 0.02),
        [0.570, 0.785, 1.000], mat4_identity());
let diffuse_specular_body = ctx.create_drawable("shader_basic",
        create_rect([-2, -1+diffuse_specular_line_y, 0], [4, 1]),
        [0.8, 0.9, 1], mat4_identity());
let n_specular_rays = 27;

let incoming_light_arrow_specular = ctx.create_drawable("shader_basic",
        create_arrow([0.6 - 0, 0.4, 0], [0 - 0, diffuse_specular_line_y, 0], [0.015, 0.03,  0.07]),
        golden, mat4_identity());
let reflected_light_arrows_specular = [];
for(let i = 0; i < n_specular_rays; i++){
    reflected_light_arrows_specular.push(ctx.create_drawable("shader_basic", null, [0.450, 0.725, 1.000], mat4_identity()));
}

let specular_lobe_border = ctx.create_drawable("shader_basic", null, [0.570, 0.785, 1.000], mat4_identity());
let specular_lobe_inside = ctx.create_drawable("shader_basic", null, [0.930, 0.965, 1.000], mat4_identity());
let roughness_specular = 25;

function update_specular_scene(){
    let specular_lobe_segments = 128;
    let specular_radius = 0.7;
    let from_specular = [-0, diffuse_specular_line_y, 0];
    let specular_center = [-0, diffuse_specular_line_y, 0];
    let specular_lobe = create_cosine_lobe(
        specular_center,
        vec3_add(specular_center, [-0.5, 0.8, 0]),
        roughness_specular, specular_lobe_segments, 0.01);
    for(let i = 0; i < n_specular_rays; i++){
        let to = specular_lobe[0][Math.floor(remap_value(i, 0, n_specular_rays, 10, specular_lobe_segments-10))];
        if(distance(to, from_specular) < 0.3){
            ctx.reset_drawable_mesh(reflected_light_arrows_specular[i]);
        }
        else{
            ctx.update_drawable_mesh(
                reflected_light_arrows_specular[i],
                create_arrow(from_specular, to, [0.015, 0.03,  0.07])
            );
        }
    }
    ctx.update_drawable_mesh(specular_lobe_border, specular_lobe[1]);
    ctx.update_drawable_mesh(specular_lobe_inside, create_polygon(specular_lobe[0]));
}

update_specular_scene();

document.getElementById("roughness-input-specular").value = roughness_specular;
document.getElementById("roughness-input-specular").addEventListener("input", (e) => {
    roughness_specular = 61-parseFloat(e.target.value);
    update_specular_scene();
});
// scene_specular
// scene_diffuse_specular
let n_diffuse_rays = 15;

let incoming_light_arrow_diffuse = ctx.create_drawable("shader_basic",
        create_arrow([0.6 + 0, 0.4, 0], [0 + 0, diffuse_specular_line_y, 0], [0.015, 0.03,  0.07]),
        golden, mat4_identity());
let reflected_light_arrows_diffuse = [];
for(let i = 0; i < n_diffuse_rays; i++){
    reflected_light_arrows_diffuse.push(ctx.create_drawable("shader_basic", null, [0.450, 0.725, 1.000], mat4_identity()));
}

let diffuse_lobe_border = ctx.create_drawable("shader_basic", null, [0.570, 0.785, 1.000], mat4_identity());
let diffuse_lobe_inside = ctx.create_drawable("shader_basic", null, [0.930, 0.965, 1.000], mat4_identity());

function update_diffuse_scene(){
    let diffuse_lobe_segments = 64;
    let diffuse_radius = 0.7;
    let from_diffuse = [0, diffuse_specular_line_y, 0];
    let diffuse_center = [0, diffuse_specular_line_y, 0];
    let diffuse_lobe = create_cosine_lobe(
        diffuse_center,
        vec3_add(diffuse_center, [0, 0.8, 0]),
        1, diffuse_lobe_segments, 0.01);
    for(let i = 0; i < n_diffuse_rays; i++){
        let to = diffuse_lobe[0][Math.floor(diffuse_lobe_segments/2) + (i - Math.floor(n_diffuse_rays / 2)) * 3];
        ctx.update_drawable_mesh(
            reflected_light_arrows_diffuse[i],
            create_arrow(from_diffuse, to, [0.015, 0.03,  0.07])
        );
    }
    ctx.update_drawable_mesh(diffuse_lobe_border, diffuse_lobe[1]);
    ctx.update_drawable_mesh(diffuse_lobe_inside, create_polygon(diffuse_lobe[0]));
}

update_diffuse_scene();
// scene_diffuse_specular
...
let refracted_light_lines_non_metal = [];
...
for(let i = 0; i < n_refract_rays; i++){
    refracted_light_lines_non_metal.push(ctx.create_drawable("shader_basic", null, [1.000, 0.587, 0.415], mat4_identity()));
    refracted_light_arrows_non_metal.push(ctx.create_drawable("shader_basic", null, [1.000, 0.587, 0.415], mat4_identity()));
}

let non_metal_particles = [];
let non_metal_particle_positions = [];
let density = 15;
function generate_non_metal_particles() {
    non_metal_particles = [];
    non_metal_particle_positions = [];
    ...
    for (let grid_x = 0; grid_x < grid_size && particle_count < num_particles; grid_x++) {
        for (let grid_y = 0; grid_y < grid_size && particle_count < num_particles; grid_y++) {
            ...
            const base_y = -1.35 + roughness_non_metal_y_offset + grid_y * cell_height + cell_height / 2;
            ...
            if (x >= -1 + radius && x <= 1 - radius &&
                ...
                non_metal_particle_positions.push([x, y]);
                particle_count++;
            }
        }
    }
...
function update_roughness_non_metal_scene(){
    ...
    if(typeof particle_visit_counts === "undefined" || particle_visit_counts.length !== non_metal_particle_positions.length){
        particle_visit_counts = new Array(non_metal_particle_positions.length).fill(0);
    }

    for(let i = 0; i < n_refract_rays; i++){
        let surface_y = -0.4 + roughness_non_metal_y_offset;
        let bottom_y = -1.4 + roughness_non_metal_y_offset;

        let from = [0, surface_y];
        let n1 = 1.0;
        let n2 = 1.5;

        let rays_spread = remap_value(roughness_non_metal, 0, 1, 0.0, 0.2);
        let reflect_angle = remap_value(i, 0, n_refract_rays-1, Math.PI/2 + rays_spread, Math.PI - rays_spread);
        let incident_angle = Math.PI/2 - reflect_angle;
        let refract_angle = Math.asin(clamp((n1 * Math.sin(incident_angle)) / n2, -0.9999, 0.9999));

        let refract_to = [Math.sin(refract_angle), -Math.cos(refract_angle)];
        refract_to = vec2_scale(vec2_normalize(refract_to), 0.2);
        let ray_end = vec2_add(refract_to, [0, surface_y]);

        let points = [];
        points.push([from[0], from[1], 0]);
        points.push([ray_end[0], ray_end[1], 0]);

        let bounce_count = 20;
        let prev = [points[points.length-2][0], points[points.length-2][1]];
        let last = [points[points.length-1][0], points[points.length-1][1]];
        let prev_dir = vec2_normalize(vec2_sub(last, prev));
        if(vec2_magnitude(prev_dir) === 0) prev_dir = vec2_normalize(refract_to);

        let visitedThisLine = new Set();

        for(let j = 0; j < bounce_count; j++){
            let t = j / Math.max(1, bounce_count-1);
            let bias_up = 0.2 + 0.6 * t;
            let search_dir = vec2_normalize(vec2_add(vec2_scale(prev_dir, 1 - bias_up), vec2_scale([0,1], bias_up)));

            let maxAngle = lerp(Math.PI/1.2, Math.PI/6, t);
            let cosThreshold = Math.cos(maxAngle);

            let maxStep = (1 + 0.5 * t);

            let nearestDist = Infinity;
            for(let k = 0; k < non_metal_particle_positions.length; k++){
                let p = non_metal_particle_positions[k];
                let toP = [p[0] - last[0], p[1] - last[1]];
                let d = vec2_magnitude(toP);
                if(d < 1e-6 || d > maxStep) continue;
            ...
            for(let k = 0; k < non_metal_particle_positions.length; k++){
                if(visitedThisLine.has(k)) continue;
                let p = non_metal_particle_positions[k];
                let toP = [p[0] - last[0], p[1] - last[1]];
                let d = vec2_magnitude(toP);
                if(d < 1e-6 || d > maxStep) continue;
                if(nearestDist !== Infinity && d > nearestDist * 1.5) continue;
                let toPN = vec2_scale(toP, 1 / d);
                let dot = vec2_dot(toPN, search_dir);
                if(!(dot >= cosThreshold || d <= nearestDist * 1.05)) continue;

                let visitCount = particle_visit_counts[k] || 0;
                let visitPenalty = 1 + 0.6 * visitCount;
                let freshBonus = visitCount === 0 ? 0.5 : 1.0;
                let score = d * visitPenalty * freshBonus;
            ...
            if(bestIdx >= 0){
                let p = non_metal_particle_positions[bestIdx];
                let next = [p[0], p[1]];
                particle_visit_counts[bestIdx] = (particle_visit_counts[bestIdx] || 0) + 1;
                visitedThisLine.add(bestIdx);
                if(next[1] < bottom_y) next[1] = bottom_y + 0.001;
                if(next[1] > surface_y) next[1] = surface_y - 0.001;
                points.push([next[0], next[1], 0]);
                prev = last;
                last = next;
                prev_dir = vec2_normalize(vec2_sub(last, prev));
                if(vec2_magnitude(prev_dir) === 0) prev_dir = vec2_normalize(refract_to);
                continue;
            }

            let step_dir = vec2_normalize(vec2_add(vec2_scale(prev_dir, 0.6), vec2_scale([0,1], 0.4)));
            let step_len = 0.1;
            if(step_len > maxStep) step_len = maxStep;
            let next = vec2_add(last, vec2_scale(step_dir, step_len));
            if(next[1] < bottom_y) next[1] = bottom_y + 0.001;
            if(next[1] > surface_y) next[1] = surface_y - 0.001;
            points.push([next[0], next[1], 0]);
            prev = last;
        ...
        let up_spread = Math.PI/6;
        let angle = Math.PI/2 + random_between(-up_spread, up_spread);
        let shoot_dir = vec2_normalize([Math.cos(angle), Math.sin(angle)]);
        if (shoot_dir[1] <= 0) shoot_dir[1] = Math.abs(shoot_dir[1]) + 0.001;

        let target_y = surface_y + 0.05;
        let t_to_surface = (target_y - last[1]) / shoot_dir[1];
        if (t_to_surface <= 0) t_to_surface = 0.2;

        let final_pt = vec2_add(last, vec2_scale(shoot_dir, t_to_surface+0.2));
        if (final_pt[0] < -1.0) final_pt[0] = -1.0;
        if (final_pt[0] > 1.0)  final_pt[0] = 1.0;
        if (final_pt[1] > surface_y + 0.2) final_pt[1] = surface_y + 0.2;

        points.push([final_pt[0], final_pt[1], 0]);

        let main_pts = points.slice(0, points.length - 1);

        ctx.update_drawable_mesh(
            refracted_light_lines_non_metal[i],
            create_line(main_pts, 0.015, false)
        );

        ctx.update_drawable_mesh(
            refracted_light_arrows_non_metal[i],
            create_arrow(points[points.length - 2], points[points.length - 1], [0.015, 0.03,  0.07])
        );
    }
}
...
function update_electric_field(scene) {
    ...
    function rk4_step(point, charges, direction = 1) {
        let k1 = calculate_field_at_point(point, charges);
        let temp = vec3_add(point, vec3_scale(k1, step_size * 0.5 * direction));
        let k2 = calculate_field_at_point(temp, charges);
        temp = vec3_add(point, vec3_scale(k2, step_size * 0.5 * direction));
        let k3 = calculate_field_at_point(temp, charges);
        temp = vec3_add(point, vec3_scale(k3, step_size * direction));
    ...
    function generate_circle_points(center, radius, num_points, start_angle = 0, end_angle = Math.PI * 2.0) {
        let points = [];
        let angle_span = end_angle - start_angle;
        let closed_loop = Math.abs(angle_span - Math.PI * 2.0) < 0.00001;
        for (let i = 0; i < num_points; i++) {
            let t = closed_loop ? i / num_points : (i + 0.5) / num_points;
            let angle = start_angle + angle_span * t;
            let x = center[0] + radius * Math.cos(angle);
            let y = center[1] + radius * Math.sin(angle);
            points.push({point: [x, y, 0], angle});
        }
        return points;
    }

    function integrate_field_line(start_point, charges, direction = 1) {
        let points = [start_point];
        let current_point = [...start_point];
        let hit_charge = null;
        for (let i = 0; i < max_steps; i++) {
            let step = rk4_step(current_point, charges, direction);
            if (vec3_magnitude(step) < 0.01) break;
            current_point = vec3_add(current_point, step);
            points.push([...current_point]);
            hit_charge = charges.find(charge => vec3_magnitude(vec3_sub(current_point, charge.pos)) < hit_radius);
            if (hit_charge) break;
            if (Math.abs(current_point[0]) > bounds_limit || Math.abs(current_point[1]) > bounds_limit) break;
        }
        return {points, end_point: current_point, hit_charge};
    }

    function push_field_line(points) {
    ...
    scene.field_lines = [];
    const positive_charge = scene.charges.find(c => c.id.includes("positive"));
    const negative_charge = scene.charges.find(c => c.id.includes("negative"));
    if (!positive_charge || !negative_charge) return;

    let negative_hit_angles = [];
    let start_points = generate_circle_points(positive_charge.pos, start_radius, lines);
    for (let start_point of start_points) {
        let result = integrate_field_line(start_point.point, scene.charges, 1);
        push_field_line(result.points);
        if (result.hit_charge && result.hit_charge.id === negative_charge.id) {
            negative_hit_angles.push(Math.atan2(
                result.end_point[1] - negative_charge.pos[1],
                result.end_point[0] - negative_charge.pos[0]
            ));
        }
    }

    if (negative_hit_angles.length === 0) return;

    let dipole_axis = vec3_normalize(vec3_sub(negative_charge.pos, positive_charge.pos));
    let outward_negative_angle = Math.atan2(dipole_axis[1], dipole_axis[0]);
    let relative_hit_angles = negative_hit_angles
        .map(angle => wrap_angle(angle - outward_negative_angle))
        .sort((a, b) => a - b);

    let lower_gap_edge = null;
    let upper_gap_edge = null;
    for (let angle of relative_hit_angles) {
        if (angle < 0) lower_gap_edge = angle;
        if (angle > 0 && upper_gap_edge === null) upper_gap_edge = angle;
    }

    if (lower_gap_edge === null) lower_gap_edge = relative_hit_angles[relative_hit_angles.length - 1] - Math.PI * 2.0;
    if (upper_gap_edge === null) upper_gap_edge = relative_hit_angles[0] + Math.PI * 2.0;

    let gap_start = lower_gap_edge + angle_margin;
    let gap_end = upper_gap_edge - angle_margin;
    if (gap_end <= gap_start) return;

    let gap_ratio = (gap_end - gap_start) / (Math.PI * 2.0);
    let extra_lines = Math.max(2, Math.ceil(lines * gap_ratio));
    let extra_start_points = generate_circle_points(
        negative_charge.pos,
        start_radius,
        extra_lines,
        outward_negative_angle + gap_start,
        outward_negative_angle + gap_end
    );

    for (let start_point of extra_start_points) {
        let result = integrate_field_line(start_point.point, scene.charges, -1);
        if (result.hit_charge && result.hit_charge.id === positive_charge.id) continue;
        push_field_line(result.points);
    }
}
...
function get_directional_light_xyz(scene){
    let directional_light_xyz = [0, 0, 0];
    directional_light_xyz[0] = Math.cos(scene.directional_light[0]) * Math.sin(scene.directional_light[1]);
    directional_light_xyz[1] = Math.sin(scene.directional_light[0]);
    directional_light_xyz[2] = Math.cos(scene.directional_light[0]) * Math.cos(scene.directional_light[1]);
    return vec3_normalize(directional_light_xyz);
}
...
function update(current_time){
    ...
    for(let scene_id in ctx.scenes){
        ...
        if(scene_id == "scene_charges"){
            ...
            ctx.draw(normal_distribution_mesh, {roughness: roughness_normal_distribution, light_direction: get_directional_light_xyz(scene)});
        }
        else if(scene_id == "scene_diffuse"){
            let directional_light_xyz = get_directional_light_xyz(scene);

            const diffuse_scene_width = scene.width;
            const diffuse_scene_height = scene.height;
            const diffuse_viewport_count = diffuse_spheres.length;
            const diffuse_gap = Math.max(0, Math.floor(diffuse_sphere_gap * pixel_ratio));
            const diffuse_viewport_height = height;
            const diffuse_requested_viewport_width = Math.max(1, Math.floor(width * (diffuse_viewport_width_percent / 100)));
            const diffuse_total_gap = diffuse_gap * Math.max(0, diffuse_viewport_count - 1);
            const diffuse_max_viewport_width = Math.max(1, Math.floor((width - diffuse_total_gap) / Math.max(1, diffuse_viewport_count)));
            const diffuse_viewport_width = Math.min(diffuse_requested_viewport_width, diffuse_max_viewport_width);
            const diffuse_used_width = diffuse_viewport_width * diffuse_viewport_count + diffuse_total_gap;
            const diffuse_left = left + Math.floor((width - diffuse_used_width) / 2);
            diffuse_labels_el.style.width = `${diffuse_used_width / pixel_ratio}px`;
            diffuse_labels_el.style.marginLeft = `${(diffuse_left - left) / pixel_ratio}px`;
            diffuse_labels_el.style.columnGap = `${diffuse_gap / pixel_ratio}px`;
            for(let i = 0; i < diffuse_labels_el.children.length; i++){
                diffuse_labels_el.children[i].style.width = `${diffuse_viewport_width / pixel_ratio}px`;
            }
            for(let i = 0; i < diffuse_spheres.length; i++){
                const diffuse_sphere = diffuse_spheres[i];
                const viewport_left = diffuse_left + i * (diffuse_viewport_width + diffuse_gap);
                const viewport_width = diffuse_viewport_width;
                const viewport_height = diffuse_viewport_height;

                const diffuse_camera = {
                    fov: scene.camera.fov,
                    z_near: scene.camera.z_near,
                    z_far: scene.camera.z_far,
                    position: [...scene.camera.position],
                    rotation: [...scene.camera.rotation],
                    up_vector: [...scene.camera.up_vector],
                    view_matrix: mat4_identity(),
                    projection_matrix: mat4_identity(),
                    orbit: {
                        rotation: [...scene.camera.orbit.rotation],
                        pivot: [0, 0, 0],
                        zoom: scene.camera.orbit.zoom
                    }
                };

                scene.width = diffuse_scene_width * (viewport_width / Math.max(1, width));
                scene.height = diffuse_scene_height * (viewport_height / Math.max(1, height));
                gl.viewport(viewport_left, bottom, viewport_width, viewport_height);
                gl.scissor(viewport_left, bottom, viewport_width, viewport_height);
                ctx.draw(diffuse_sphere.current_drawable, {light_direction: directional_light_xyz, diffuse_type: diffuse_sphere.diffuse_type, roughness: roughness_oren_nayar, offset: [0, 0, 0]}, diffuse_camera);
            }

            scene.width = diffuse_scene_width;
            scene.height = diffuse_scene_height;
            gl.viewport(left, bottom, width, height);
            gl.scissor(left, bottom, width, height);
        }
        else if(scene_id == "scene_geometric_function"){
            ctx.draw(geometric_function_mesh, {roughness: roughness_geometric_function, light_direction: get_directional_light_xyz(scene)});
        }
        else if(scene_id == "scene_fresnel_equation"){
            ctx.draw(fresnel_equation_mesh, {base_reflectance: base_reflectance, light_direction: get_directional_light_xyz(scene)});
        }
        else if(scene_id == "scene_pbr_demo"){
            ctx.draw(pbr_demo_mesh, {albedo: albedo_pbr_demo, metallic: metallic_pbr_demo, roughness: roughness_pbr_demo, light_direction: get_directional_light_xyz(scene)});
        }
        else if(scene_id == "scene_pbr_demo_grid"){
            for(let i = 0; i < pbr_demo_grid_spheres.length; i++){
                ctx.draw(pbr_demo_grid_spheres[i].current_drawable, {albedo: albedo_pbr_demo, metallic: pbr_demo_grid_spheres[i].metallic, roughness: pbr_demo_grid_spheres[i].roughness, light_direction: [2, 1, 2]});
            }
        ...
        else if(scene_id == "scene_specular"){
            ctx.draw(diffuse_specular_line);
            ctx.draw(diffuse_specular_body);

            for(let i = 0; i < n_specular_rays; i++){
                ctx.draw(reflected_light_arrows_specular[i]);
            }
            ctx.draw(incoming_light_arrow_specular);
            ctx.draw(specular_lobe_border);
            ctx.draw(specular_lobe_inside);
        }
        else if(scene_id == "scene_diffuse_lobe"){
            ctx.draw(diffuse_specular_line);
            ctx.draw(diffuse_specular_body);

            for(let i = 0; i < n_diffuse_rays; i++){
                ctx.draw(reflected_light_arrows_diffuse[i]);
            }
            ctx.draw(incoming_light_arrow_diffuse);
            ctx.draw(diffuse_lobe_border);
            ctx.draw(diffuse_lobe_inside);
        }
            ...
            for(let i = 0; i < n_refract_rays; i++){
                ctx.draw(refracted_light_lines_non_metal[i]);
            ...
            gl.drawElements(gl.TRIANGLES, fullscreen_quad.vertex_buffer.draw_count, gl.UNSIGNED_INT, 0);
        }
...
const meshes = [
    ...
    { path: "stanford-bunny.mesh", drawable: normal_distribution_bunny },
    ...diffuse_spheres.map((diffuse_sphere) => ({ path: "stanford-bunny.mesh", drawable: diffuse_sphere.bunny_drawable })),
    { path: "stanford-bunny.mesh", drawable: geometric_function_bunny },
    { path: "stanford-bunny.mesh", drawable: fresnel_equation_bunny },
    { path: "stanford-bunny.mesh", drawable: pbr_demo_bunny },
    ...pbr_demo_grid_spheres.map((sphere) => ({ path: "stanford-bunny.mesh", drawable: sphere.bunny_drawable })),
];
